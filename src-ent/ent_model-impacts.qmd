...
get_TgtN <- function(dat, subN){
  ...
  ests <- rbind(ests, tibble(t=1:(swch_idx[1]-1),
                     TgtN = rep(1, length(1:(swch_idx[1]-1)))))
...
Again, dividing by the trial number gives a nice approximation to the rate

```{r}

dat %>% group_by(t) %>% summarise(tgtN = TgtN[1]) %>% mutate(tr = tgtN/t) %>% 
  ggplot(aes(x=t, y=tr)) + geom_line()
```

**Problem is this is highly correlated with the number of switches regressor**
...
Will come back to this. Could make a probability vector anyway.

**Next one is**: People track the probability of the context, given the number of non-target doors they have selected from the current context, since selection of the last target
