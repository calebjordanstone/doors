...
with(dat, cor(Sw, TgtN)) # check correlation between regressors
```
...
Here is the formula in a function:
...
get_p_context <- function(dat, subN){
  ...
  pA <- 0
  pB <- 0
  for (i in 1:(length(cntx)-1)){ # -1 reflects we won't apply an update after the last trial
    pA <- c(pA, sum(cntx[1:i] < 2)/i)
    pB <- c(pB, sum(cntx[1:i] > 1)/i)
  }
  ts <- with(tmp, unique(t))
  tmp <- inner_join(tmp, tibble(t = ts, pC1 = pA, pC2 = pB), by="t") 
  ####################################
  
  ######################################
  # now I need for each trial, the number of n's (fails from the same context as the previous), and the number of m's (fails from the different 
  # context as the previous)
  ...
  tmp$door_n <- 0
  tmp$door_m <- 0
  tmp$door_n[tmp$context == tmp$prv_cntx] <- tmp$door_cc[tmp$context == tmp$prv_cntx]
  # row 1 from comments above
  tmp$door_n[tmp$context != tmp$prv_cntx] <- tmp$door_oc[tmp$context != tmp$prv_cntx] # row 2 from comments above 
  tmp$door_m[tmp$context == tmp$prv_cntx & tmp$prv_cntx > 0] <- 
    tmp$door_oc[tmp$context == tmp$prv_cntx & tmp$prv_cntx > 0] # row 3
  tmp$door_m[tmp$context != tmp$prv_cntx  & tmp$prv_cntx > 0] <- 
    tmp$door_cc[tmp$context != tmp$prv_cntx & tmp$prv_cntx > 0] # row 4
#####NOTE: door_m is also our DV
################################################################
...
#############################################################
  ...
  tmp <- tmp %>% group_by(t) %>% mutate(sum_n = cumsum(door_n),
                                        sum_m = cumsum(door_m)) 
#############################################################
  
#############################################################
  # now make 2 new variables, that redistribute pC1 and pC2 to be relative 
  # to the context in which the last participant was rewarded. e.g. if my last
  # reward was in context 2, then I want the probability of being in pC2 for the 
  # previous context variable, and pC1 for the other context variable
  tmp$p_prv_cntxt <- 0 # establish variables
  tmp$p_oth_cntxt <- 0
  tmp$p_prv_cntxt[tmp$prv_cntx == 1] <- tmp$pC1[tmp$prv_cntx == 1]
  tmp$p_prv_cntxt[tmp$prv_cntx == 2] <- tmp$pC2[tmp$prv_cntx == 2]
  tmp$p_oth_cntxt[tmp$prv_cntx == 1] <- tmp$pC2[tmp$prv_cntx == 1]
  tmp$p_oth_cntxt[tmp$prv_cntx == 2] <- tmp$pC1[tmp$prv_cntx == 2]
 #############################################################
...
#############################################################
  ...
  tmp$pNgCr <- p_vals[as.factor(tmp$sum_n)]
  tmp$pMgOc <- p_vals[as.factor(tmp$sum_m)]
  tmp$pNgCr[is.na(tmp$pNgCr)] <- 0
  tmp$pMgOc[is.na(tmp$pMgOc)] <- 0
#############################################################

#############################################################
  ##### now work out the probability of the context, given n & m
  tmp <- tmp %>% mutate(N_oc = pNgCr*p_prv_cntxt/  (pNgCr*p_prv_cntxt + pMgOc*p_oth_cntxt))
#############################################################  
  tmp
}
```
