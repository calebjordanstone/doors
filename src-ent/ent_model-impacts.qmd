...
#############################################################
  ...
  tmp <- tmp %>% mutate(p_Scgn = (p_n_g_sC*psC /  (p_n_g_sC*psC + p_m_g_dC*pdC)),
                        p_Ocgn = 1 - p_Scgn)
  # now for every situation of 1 vs 0, I am going to add a tiny amount to
  # the 0, and subtract that from the 1, to reflect a little uncertainty
  psC_zero_idx = which(tmp$p_Scgn == 0)
  poC_zero_idx = which(tmp$p_Ocgn == 0)
  uncert = .01
  tmp$p_Scgn[c(psC_zero_idx, poC_zero_idx)] = 
        c(tmp$p_Scgn[psC_zero_idx] + uncert, tmp$p_Scgn[poC_zero_idx] - uncert)
  tmp$p_Ocgn[c(poC_zero_idx, psC_zero_idx)] = 
        c(tmp$p_Ocgn[poC_zero_idx] + uncert, tmp$p_Ocgn[psC_zero_idx] - uncert)
...
#############################################################
  ...
  # replace any infs with zeros
  tmp <- tmp %>% mutate(p_succss_sC = if_else(is.infinite(p_succss_sC), uncert, p_succss_sC),
                        p_succss_oC = if_else(is.infinite(p_succss_oC), uncert, p_succss_oC),
...
ggpairs(dat %>% filter(sum_m != 4) %>% select(Sw, Sw_r, cntxt_odds, succss_odds))
```
...
fit <- glm(door_m ~ scale(Sw) + scale(Sw_r) + scale(succss_odds) + scale(cntxt_odds), data=dat %>% filter(sum_m != 4), family=binomial(link="logit"))
summary(fit)
```
