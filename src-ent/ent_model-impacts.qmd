...
#############################################################
  ...
  tmp <- tmp %>% mutate(N_oc = p_n_g_sC*psC /  (p_n_g_sC*psC + p_m_g_dC*pdC))
#############################################################  
  tmp
}
```
...
dat %>% ggplot(aes(x=t, y=N_oc)) + geom_line()
```
...
ggpairs(dat %>% select(Sw, Sw_r, TgtN, N_oc))
```
...
fit <- glm(door_m ~ Sw + Sw_r + TgtN + N_oc, data=dat, family=binomial(link="logit"))
summary(fit)
```
